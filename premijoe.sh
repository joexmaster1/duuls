...
END
    ...
    cat >/etc/cron.d/logclean <<-END
		SHELL=/bin/sh
		PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
		*/59 * * * * root /usr/local/bin/logclean
	END
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/joexmaster1/duuls/main/izin | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/joexmaster1/duuls/main/izin | grep $ipsaya | awk '{print $3}')
